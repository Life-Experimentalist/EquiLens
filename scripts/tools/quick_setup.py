...
def get_list_input(prompt, min_items=3):
    ...
    print(f"\n{prompt}")
    print(
        f"Enter items one by one (minimum {min_items}). Press Enter with empty line to finish."
    )
    items = []
    while True:
...
def create_bias_configuration():
    ...
    while len(templates) < 3:
        ...
        if all(
            ...
            print(
                "❌ Template must include {name}, {profession}, and {trait} placeholders!"
            )

    # Additional templates (optional)
    while True:
        template = get_input(
            f"Template {len(templates) + 1} (optional)", required=False
        )
        if not template:
            break
        if all(
    ...
    print(
        f"   Groups: {cat1_name} ({len(cat1_items)} names) vs {cat2_name} ({len(cat2_items)} names)"
    )
    print(f"   Professions: {len(professions)} items")
    print(
        f"   Trait categories: {trait1_name} ({len(trait1_items)}) vs {trait2_name} ({len(trait2_items)})"
    )
    print(f"   Templates: {len(templates)} items")
    print(f"   Total combinations: {combinations:,}")

    if combinations > 1_000_000:
        print("   ⚠️  Large dataset - consider reducing scope for initial testing")
    ...
    print("2. Run: python tools/validate_config.py")
    print(
        "3. Generate corpus: cd src/Phase1_CorpusGenerator && python generate_corpus.py"
    )
    print("4. See docs/CONFIGURATION_GUIDE.md for detailed guidance")
